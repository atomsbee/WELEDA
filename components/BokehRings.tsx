...
// 22 rings + filled soft dots
// Each has unique: size, left%, delay, duration, color
const RINGS = [
  ...
  // Second wave — slightly different sizes + timings
  ...
  // --- Filled soft bokeh dots (glowing blobs) ---
  { size: 9,   left: '13%', delay: '10s',   dur: '26s', color: 'rgba(180,120,255,0.50)', filled: true  },
  { size: 6,   left: '33%', delay: '15s',   dur: '22s', color: 'rgba(255,100,180,0.55)', filled: true  },
  { size: 11,  left: '54%', delay: '3s',    dur: '24s', color: 'rgba(255,200,70,0.45)',  filled: true  },
  { size: 7,   left: '74%', delay: '18s',   dur: '20s', color: 'rgba(60,220,180,0.50)',  filled: true  },
  { size: 5,   left: '91%', delay: '8s',    dur: '18s', color: 'rgba(255,255,255,0.40)', filled: true  },
  { size: 8,   left: '22%', delay: '20s',   dur: '23s', color: 'rgba(255,100,180,0.45)', filled: true  },
  { size: 10,  left: '63%', delay: '12s',   dur: '25s', color: 'rgba(180,120,255,0.42)', filled: true  },
  { size: 6,   left: '80%', delay: '6s',    dur: '19s', color: 'rgba(255,200,70,0.48)',  filled: true  },
]
...
export default function BokehRings() {
  ...
  // Only show in dark mode, after mount (avoid SSR flash)
      ...
      {RINGS.map((r, i) => (
        <div
          ...
          style={{
            ...
            left:       r.left,
            bottom:     `-${r.size + 10}px`,
            // Outline rings: transparent fill + colored border
            // Filled dots: radial gradient glow, no border
            border:     r.filled
              ? 'none'
              : `1px solid ${r.color}`,
            background: r.filled
